...
#%%
def beoordeling_categorie_jurylid(categorie, jurylid, sheet_name="Beoordeling"):
...
if not st.session_state['logged_in']:
    ...
    @st.cache_data(ttl=300)
    def load_programma():
        df = pd.read_excel("Programma stoetopstellers 2024.xlsx", nrows=63)
        return df
    programma_df = load_programma()
    programma_df = programma_df.iloc[:, :16] # tot kolom P (16e)
